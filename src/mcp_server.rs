...
Use tools.tmux for persistent terminal sessions: tools.tmux.open(name, { cwd, command }), tools.tmux.run(name, 'ls') to run a command and capture its stdout/exitCode, tools.tmux.send/capture/close. \
